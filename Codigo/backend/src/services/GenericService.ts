...
class GenericService {
    ...
    update(
        ...
        if (service !== TablesNames.USERS) {
            queryOptions.filters = { "id": String(targetId), "created_by": access.userId };
        } else {
    ...
    remove(
        ...
        if (service !== TablesNames.USERS) {
            ...
            queryOptions.filters = { "id": String(targetId) };
        }
    ...
    getLastInsertedItem = (service: TablesNames, userId: string): Promise<[{ id: number }]> => {
